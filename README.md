...
**검색 품질 테스트는 로컬 임베딩 실물을 씁니다.** "기대한 문서의 청크가 1위로 오는가"는 대역으로는 확인할 수 없습니다 — 해시 기반 페이크 벡터에는 의미가 없어 1위가 무엇이든 정상으로 보입니다. 그래서 이 층만 실제 모델을 쓰고, 가중치가 호스트에 없으면 건너뜁니다. **이미지에는 가중치가 구워져 있으므로** 컨테이너 안에서는 실행됩니다:

```bash
make test-container   # 테스트 스테이지 이미지를 빌드해 그 안에서 전부 실행
```

세 명령이 덮는 범위가 다릅니다.

| 명령 | 구조 층 (필터·순서·경계·오류) | 실물 Chroma 어댑터 | 검색 품질 (임베딩 실물) | 필요한 것 |
|------|:---:|:---:|:---:|------|
| `make test` | ✅ | 건너뜀 | 가중치가 캐시돼 있으면 ✅ | 없음 |
| `make test-all` | ✅ | ✅ | 가중치가 캐시돼 있으면 ✅ | Docker (벡터 스토어) |
| `make test-container` | ✅ | 건너뜀 (컨테이너에서 `localhost:8001`이 벡터 스토어가 아닙니다) | ✅ | Docker |

어느 쪽이든 **LLM 구독도 API 키도 필요 없습니다.** 세 명령 모두 초록이어야 하고, 건너뛴 항목은 실행 결과에 사유와 함께 표시됩니다.
