...
Docker와 Docker Compose만 있으면 됩니다. 컨테이너 셋(API · 벡터 스토어 · 캐시 저장소)이 함께 뜨고, 벡터 스토어와 문서 레지스트리는 각각 볼륨에 영속화됩니다.

| 서비스 | 이미지 | 호스트 포트 |
|---|---|---:|
| `api` | 이 리포 | 8000 |
| `vector-store` | `chromadb/chroma:1.5.9` | 8001 |
...
벡터 스토어는 별도 서비스라 도커 없이 대체할 수 없습니다. 없이 띄우면 서비스는 정상 기동하고 헬스가 `unavailable`로 보고하며, 문서 업로드는 `503`이 됩니다.
