...
- [x] 8.6 LLM-Judge 판정 통과 수를 두 구성에서 재고 **`판정불가` 수를 함께** 적는다. 판정불가가 과반인 회차의 통과 수는 읽지 않는다 (design 결정 11·13)

## 9. 품질 테스트와 회귀

- [x] 9.1 리랭킹 순위 품질을 골든셋으로 잰다 — 켠 구성과 끈 구성을 **같은 저장소 위에서**(`searching_with`) 비교해 근거 인용문의 `recall@k`·MRR 과 순위가 움직인 문항을 찍는다. 가중치가 없으면 사유와 함께 건너뛴다(임베딩 품질 테스트와 같은 방식)
- [x] 9.2 회귀 — `sample-docs/` 네 질의의 1위가 리랭커를 켠 구성과 끈 구성에서 같은지 (`retrieval`: 리랭킹을 켜도 회귀 질의의 1위가 같다)
- [x] 9.3 회귀 — 하이브리드 구제 시나리오(식별자 질의)가 리랭킹을 켜도 그대로인지
- [x] 9.4 회귀 — 리랭커를 끈 구성에서 **기존 테스트 전부**가 통과하는지. 이것이 "얹었을 뿐 바꾸지 않았다"의 증명이다
- [x] 9.5 `python3 scripts/check_comments.py` 로 주석 규칙 위반 0건을 확인한다. 새 파일의 긴 설명은 `tests/README.md` 나 `ARCHITECTURE.md` 로 옮긴다
- [x] 9.6 `tests/llm_judge.py` — 판정 프롬프트와 구조화 파서. 판정자에게 질문·`reference_answer`·생성된 답변만 넘기고 **근거 청크는 넘기지 않는다**. 출력은 `{verdict, reason}` 하나이고, 형식 위반·파싱 실패는 오답이 아니라 `판정불가`로 센다 (design 결정 13)
- [x] 9.7 골든셋 채점을 세 층으로 나눈다 — ① 검색 게이트(`evidence.quote` 가 상위 K 에 왔는가) → ② 결정적 문자열 검사(`expected_spans`·`must_not_contain`, 공백 제거 후) → ③ LLM-Judge. **게이트를 통과하지 못한 문항은 생성 채점에서 빼고 검색 실패로만 센다**. ②와 ③은 합산하지 않고 나란히 적는다 (골든셋 README, design 결정 13)
- [x] 9.8 `answerable: false` 6문항은 검색 게이트 없이 판정한다 — 답변이 근거 없음을 밝혔는지. 리랭킹이 환각 억제를 나쁘게 만들지 않았는지를 이 여섯 문항이 잡는다
- [x] 9.9 `qa_harness` 를 **실물 생성기**로 세울 수 있게 넓힌다 — 지금은 `ScriptedGenerator` 만 받는다. 리랭커를 받는 검색 하네스 위에 얹어 두 구성의 상위 K 로 각각 답변을 만든다. 첫 회차는 **두 구성 모두 근거가 온 문항(교집합)** 으로 재고 두 집합의 크기를 함께 찍는다 (design Open Questions)
- [x] 9.10 이 층을 `-m "llm and slow"` 로만 돌게 하고 `pyproject.toml` 의 마커 설명을 갱신한다. `addopts` 의 기본 제외는 그대로다 — 평가자가 도는 한 줄이 구독에 묶이면 안 된다
- [x] 9.11 **품질 단언을 두지 않는다.** 테스트가 단언하는 것은 하네스 건전성 하나 — `판정불가`가 과반이면 실패한다. 회귀는 9.2~9.4 의 결정적 층이 진다 (design 결정 13)

## 10. 문서

- [x] 10.1 `ARCHITECTURE.md` 「재랭킹·질의 확장은 여전히 없다」를 **리랭킹 절로 갈아 쓴다.** 파이프라인 그림에 단계를 더하고, 질의 확장·HyDE 가 여전히 없다는 사실은 남긴다
- [x] 10.2 모델 후보표와 기각 사유를 적는다 — 기각의 축이 둘("한국어가 학습 분포 안에 있는가", "아키텍처가 `transformers` 안에 있는가")이라는 것과, 근거가 벤치마크가 아니라 학습 언어 구성·실물 로딩 결과라는 사실을 함께 적는다 (design 결정 1)
- [x] 10.3 순위 신호가 둘인 이유와 `score` 를 갈아치우지 않은 근거를 적는다 (design 결정 9)
- [x] 10.4 리랭킹 점수에 하한을 걸지 않은 이유를 적는다 — 「하한이 판정하는 것과 하지 않는 것」 옆에 둔다 (design 결정 5)
- [x] 10.5 8장의 실측표(지연·골든셋 순위 지표·판정 통과 수·이미지 크기)를 적는다. **여전히 말할 수 없는 것 세 줄을 나란히 적는다** — 문서 한 건 위의 값이라는 것, 판정자 자신이 검증되지 않았다는 것, 작은 차이는 신호가 아니라는 것 (design 결정 11)
- [x] 10.6 원격 코드를 쓰는 모델을 피한 근거와 리비전 고정을 적는다 — 첫 후보가 실물에서 깨진 기록(`transformers` 판 비호환, 코드가 다른 저장소라 고정 불가)을 함께 남긴다 (design 결정 1·8)
- [x] 10.7 캐시 키에 리랭커 몫만 들어갔고 **retriever 가중치는 여전히 키에 없다**는 사실을 적는다. 적지 않으면 다음 사람이 "검색 구성이 전부 키에 있다"고 읽는다 (design 결정 7)
- [x] 10.8 `README.md` 에 리랭커 설정(`APP_RERANKER_ENABLED`·`APP_RERANKER_MODEL`·`APP_RERANK_CANDIDATES`)과 끄는 방법, 첫 빌드가 길어진다는 사실을 적는다
- [x] 10.9 새 테스트의 밀려난 설명을 `tests/README.md` 에 파일별로 옮긴다. **`golden_harness.py` 절의 "재는 것은 근거 인용문의 순위 하나다"를 고친다** — 판정 층이 붙으면 더 이상 사실이 아니고, 그 자리에 세 층의 채점 순서와 둘을 합산하지 않는 이유를 적는다
- [x] 10.10 `PROMPT_DESIGN.md` 에 판정 프롬프트 절을 더한다 — 제품 프롬프트와 목적이 다르다는 것, 판정 출력 형식, 판정자에게 근거 청크를 넘기지 않는 이유, judge 를 게이트로 쓰지 않는 이유 (design 결정 13)
- [x] 10.11 `README.md` 의 테스트 절에 판정 층을 돌리는 명령(`-m "llm and slow"`)과 그것이 **구독을 요구해 기본 한 줄에서 빠진다**는 사실을 적는다
- [x] 10.12 `openspec validate add-cross-encoder --strict` 와 문서-코드 일치를 마지막으로 확인한다 — 문서에 적은 기능의 코드가 전부 있는지
