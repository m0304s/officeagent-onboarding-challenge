...
- [ ] 8.6 LLM-Judge 판정 통과 수를 두 구성에서 재고 **`판정불가` 수를 함께** 적는다. 판정불가가 과반인 회차의 통과 수는 읽지 않는다 (design 결정 11·13)

## 9. 품질 테스트와 회귀

- [ ] 9.1 리랭킹 순위 품질을 골든셋으로 잰다 — 켠 구성과 끈 구성을 **같은 저장소 위에서**(`searching_with`) 비교해 근거 인용문의 `recall@k`·MRR 과 순위가 움직인 문항을 찍는다. 가중치가 없으면 사유와 함께 건너뛴다(임베딩 품질 테스트와 같은 방식)
...
- [ ] 9.6 `tests/llm_judge.py` — 판정 프롬프트와 구조화 파서. 판정자에게 질문·`reference_answer`·생성된 답변만 넘기고 **근거 청크는 넘기지 않는다**. 출력은 `{verdict, reason}` 하나이고, 형식 위반·파싱 실패는 오답이 아니라 `판정불가`로 센다 (design 결정 13)
- [ ] 9.7 골든셋 채점을 세 층으로 나눈다 — ① 검색 게이트(`evidence.quote` 가 상위 K 에 왔는가) → ② 결정적 문자열 검사(`expected_spans`·`must_not_contain`, 공백 제거 후) → ③ LLM-Judge. **게이트를 통과하지 못한 문항은 생성 채점에서 빼고 검색 실패로만 센다**. ②와 ③은 합산하지 않고 나란히 적는다 (골든셋 README, design 결정 13)
- [ ] 9.8 `answerable: false` 6문항은 검색 게이트 없이 판정한다 — 답변이 근거 없음을 밝혔는지. 리랭킹이 환각 억제를 나쁘게 만들지 않았는지를 이 여섯 문항이 잡는다
- [ ] 9.9 `qa_harness` 를 **실물 생성기**로 세울 수 있게 넓힌다 — 지금은 `ScriptedGenerator` 만 받는다. 리랭커를 받는 검색 하네스 위에 얹어 두 구성의 상위 K 로 각각 답변을 만든다. 첫 회차는 **두 구성 모두 근거가 온 문항(교집합)** 으로 재고 두 집합의 크기를 함께 찍는다 (design Open Questions)
- [ ] 9.10 이 층을 `-m "llm and slow"` 로만 돌게 하고 `pyproject.toml` 의 마커 설명을 갱신한다. `addopts` 의 기본 제외는 그대로다 — 평가자가 도는 한 줄이 구독에 묶이면 안 된다
- [ ] 9.11 **품질 단언을 두지 않는다.** 테스트가 단언하는 것은 하네스 건전성 하나 — `판정불가`가 과반이면 실패한다. 회귀는 9.2~9.4 의 결정적 층이 진다 (design 결정 13)
...
- [ ] 10.9 새 테스트의 밀려난 설명을 `tests/README.md` 에 파일별로 옮긴다. **`golden_harness.py` 절의 "재는 것은 근거 인용문의 순위 하나다"를 고친다** — 판정 층이 붙으면 더 이상 사실이 아니고, 그 자리에 세 층의 채점 순서와 둘을 합산하지 않는 이유를 적는다
- [ ] 10.10 `PROMPT_DESIGN.md` 에 판정 프롬프트 절을 더한다 — 제품 프롬프트와 목적이 다르다는 것, 판정 출력 형식, 판정자에게 근거 청크를 넘기지 않는 이유, judge 를 게이트로 쓰지 않는 이유 (design 결정 13)
- [ ] 10.11 `README.md` 의 테스트 절에 판정 층을 돌리는 명령(`-m "llm and slow"`)과 그것이 **구독을 요구해 기본 한 줄에서 빠진다**는 사실을 적는다
- [ ] 10.12 `openspec validate add-cross-encoder --strict` 와 문서-코드 일치를 마지막으로 확인한다 — 문서에 적은 기능의 코드가 전부 있는지
