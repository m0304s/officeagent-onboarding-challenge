...
이 셋이 저장된 벡터의 정체성 전부라, 규칙이 흔들리면 재업로드 교체도 캐시 무효화도 근거를
잃는다. 각각이 무엇에 반응하고 무엇에 반응하지 않는지를 고정한다.
"""
...
def test_document_id_ignores_unicode_normalization_form():
    """macOS(NFD)와 리눅스(NFC)에서 같은 한글 파일명이 다른 문서가 되면 재업로드가 교체로
    이어지지 않는다."""
    nfc = "사내규정.txt"  # NFC
...
def test_each_material_changes_the_signature(material, changed):
    ...
    안 그러면 구 구성 벡터가 남은 채 캐시가 낡은 답변을 계속 내보낸다."""
...
def test_material_boundaries_are_unambiguous():
    """한 재료에 구분자를 흉내 낸 문자가 섞여도 다른 재료로 새지 않는다.

    같은 서명을 받으면 재색인이 일어나지 않아, 서명이 막으려던 실패가 서명을 통해 돌아온다."""
...
def test_signature_does_not_depend_on_document_content_or_filename():
    ...
    내용·파일명이 재료가 되면 "지금 구성이 저장 당시와 같은가"를 물을 수 없다."""
...
def test_the_tokenizer_configuration_reaches_the_signature():
    ...
    옛 규약으로 쓴 색인에 새 규약의 질의가 와, 오류 없이 아무것도 찾지 못한다."""
    baseline = derive_index_signature(**BASE_SIGNATURE_MATERIALS)
...
def test_chunk_id_separates_revisions_and_signatures():
    ...
    id 에 서명이 없으면 새 청크가 이전 것을 덮어써 되돌릴 원본이 사라진다."""
...
# ── 레코드가 스스로 하는 판정 ────────────────────────────────────────────
# 판정이 두 서비스에 따로 있으면 어긋나도 각자 기준으로는 옳아 오류가 남지 않는다.
...
def test_a_stale_document_matches_the_index_but_is_not_searchable():
    ...
    그래서 서명만 보는 판정으로는 걸러지지 않고, 빈 결과의 이유를 진단할 수 없게 된다."""
