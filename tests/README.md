...
재는 것은 **근거 인용문의 순위 하나**다. 골든셋에는 `expected_spans`·`must_not_contain`
같은 생성 채점 필드가 함께 있지만 그것은 LLM 이 있어야 성립하고, 골든셋 README 가 검색
실패와 생성 실패를 섞어 채점하지 말라고 못 박는다. 리랭킹은 결과 집합을 바꾸지 않고 순서만
바꾸므로, 순위 지표가 이 단계의 효과를 정확히 잰다 — 생성을 함께 재면 두 지표가 함께 움직여
어느 쪽이 움직인 것인지 알 수 없게 된다.
