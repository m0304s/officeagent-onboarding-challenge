...
생성기가 넷이고, 각각이 지키는 것이 다르다.

| 생성기 | 만드는 것 | 무엇을 지키나 |
|---|---|---|
| `make_pdf` | 쪽마다 텍스트, `BLANK_PAGE` 는 도형만 | 쪽 번호, 텍스트 없는 쪽의 판정 |
| `make_structured_pdf` | 조판 크기 셋 + 격자로 그린 표 | 헤딩 레벨과 마크다운 표 |
| `make_scanned_pdf` | 글자를 이미지로 구운 쪽 | 스캔본이 `no_extractable_text` 로 남는 것 |
| `make_layered_and_scanned_pdf` | 문자 레이어와 이미지가 한 쪽에 | **문서 사이 오염**의 회귀 |

**조판 크기를 넉넉히 벌리는 이유**는 헤딩 판정이 폰트 크기 분포로 갈리기 때문이다. 붙여
놓으면 두 제목이 같은 레벨로 접히고 본문까지 제목으로 승격된다(실측).

**뒤의 둘은 이미지가 든 PDF 다.** 라이브러리의 레이아웃 경로는 그런 문서를 처리한 뒤
다음 문서의 추출을 훼손하는데, 우리는 그 경로를 끄고 고전 경로에 고정했다. 이 픽스처가
없으면 누군가 레이아웃을 다시 켜도 아무 테스트도 알아채지 못한다.

**실물 스캔본은 커밋하지 않는다.** 실명·계좌번호가 든 문서는 히스토리에 남으면 지울 수
없고, 이 리포는 외부 평가자가 열람한다. 실물 확인은 로컬 파일로만 한다.
...
**PDF 계약 테스트는 두 구현으로 파라미터화한다** (`pdf_parser` 픽스처). 공통 로직을 상위
클래스나 헬퍼로 뽑지 않은 것은 의도다. 상속으로 묶으면 코드 중복은 줄지만 두 라이브러리의
실패 표현이 다른 지점에서 훅이 늘어나고, 결국 분기가 상위 클래스로 옮겨 간다. 지켜야 하는
것은 **구현의 공유가 아니라 관측되는 계약의 동일성**이고, 그것을 직접 단언하는 것이
테스트다. 그래서 쪽 번호·오류 코드·예외 차단·오염 없음이 두 파서에 각각 걸린다.

구조 보존(헤딩 레벨·표·원문 보존)은 `markdown` 방식에만 걸린다 — 그것이 두 방식이 갈리는
지점이라, 두 파서에 함께 걸면 무엇이 다른지를 테스트가 말하지 못한다.
...
### `test_reranking_core.py`

재정렬의 **정책** — 순서·결정성·집합 보존, 그리고 쓸 수 없는 점수의 처분.

점수를 손으로 주는 이유는 "관련 있는 것이 위로 오는가"가 모델의 성질이라 실물로만
확인되기 때문이다. 그것은 `test_reranking_quality.py` 가 잰다. 여기서 고정하는 것은 모델이
무엇을 돌려주든 서비스가 지켜야 하는 규칙이다.

**집합 보존이 이 파일에서 가장 무거운 단언이다.** 리랭킹은 순서를 바꿀 뿐 목록의 구성을
바꾸지 않는다는 것이 검색 파이프라인의 계약인데, 여기가 깨지면 걸러 내는 일이 채택 판정과
리랭킹 둘로 흩어진다. 깊이 밖 후보를 버리지 않고 융합 순서 그대로 뒤에 붙인다는 단언도 같은
계약이다 — 그 자리는 현재성 재검증이 앞쪽을 떨어뜨렸을 때 K 를 채운다. `rerank_score` 를
갖지 않는 것은 그 후보가 실제로 리랭커를 거치지 않았다는 사실을 값이 그대로 말하는 것이다.

점수 개수 불일치와 `NaN` 을 오류로 세우는 이유는 둘 다 **조용한 오답**이 되기 때문이다 —
개수가 어긋나면 남의 점수가 실리고, `NaN` 은 비교가 항상 거짓이라 순서가 임의가 된다.
어느 쪽도 예외를 내지 않고 그럴듯한 순서를 돌려준다.

### `test_reranking.py`

리랭킹 **어댑터** — 로딩 없이 서명을 읽는가, 점수 규약이 새지 않는가, 선언을 지키는가.

서명을 가중치 없이 읽을 수 있어야 하는 이유는 그것이 캐시 키의 재료라서다. 읽는 데 모델
로딩이 필요하면 캐시 키 유도가 2.2GB 로딩에 묶인다. 리비전이 다르면 서명도 달라야 한다는
단언이 여기 있는 것도 같은 이유다 — 같은 이름의 다른 가중치가 같은 캐시 항목을 쓰면 옛
모델이 고른 근거가 새 모델의 답인 척 남는다.

**선언 대조가 로딩을 막는 층이다.** 입력 창이 우리가 적은 것보다 좁거나, 읽을 수 없거나,
다중 레이블이거나, 우리가 설정하지 않은 활성화를 쓰는 모델은 기동에서 세운다. 이런 모델은
오류를 내지 않고 **조용히 다른 점수 규약으로** 동작하므로 런타임에는 관측되지 않는다.

실물 가중치를 쓰는 단언은 파일 끝의 둘뿐이고 없으면 건너뛴다. 나머지는 대역 모델로 돌아
`pytest` 한 줄이 다운로드에 묶이지 않는다.

### `golden_harness.py` · `test_reranking_quality.py`

리랭킹 품질 — 크로스인코더를 켠 구성과 끈 구성을 같은 저장소 위에서 비교한다.

**같은 저장소 위인 것이 이 파일의 전부다.** 앱을 두 벌 세우면 청크도 임베딩도 다시 만들어져
"리랭킹이 순위를 어떻게 바꿨는가"가 "두 색인이 어떻게 다른가"와 섞인다. `searching_with`
가 저장소를 물려받은 채 설정만 갈아 끼우는 이유가 여기 있다.

이 파일이 재는 것은 **근거 인용문의 순위 하나**다. 리랭킹은 결과 집합을 바꾸지 않고 순서만
바꾸므로 순위 지표가 이 단계의 효과를 곧바로 잰다. 답변이 좋아졌는지는 다른 파일
(`test_golden_judge.py`)이 재고, 둘을 한 파일에 두지 않는 이유는 구독 유무가 다르기
때문이다 — 순위는 `-m slow`, 판정은 `-m "llm and slow"` 다.

**대조 문자열은 공백을 지우고 비교한다.** 이 PDF 는 추출에서 한국어 어절 공백이 소실되고
(`보험회사를위하여계약체결을중개또는대리하는것`), 골든셋의 `evidence.quote` 는 그 추출
결과 그대로다. 정규화 없이는 어느 쪽도 매칭되지 않는다.

**리랭킹이 축소되지 않았는지를 매 질의마다 단언한다.** 타임아웃에 걸리면 융합 순서가
돌아오는데 그것은 정상 경로라 조용하다 — 단언이 없으면 "리랭킹을 켰는데 차이가 없다"는
결론이 실은 "리랭킹이 한 번도 돌지 않았다"일 수 있다. 제한 시간을 실측의 100배로 잡아 둔
것도 같은 이유다.

가중치 둘(임베딩·리랭커)이 전부 필요해 **없으면 파일 전체가 건너뛰어진다.** 컨테이너
이미지에는 둘 다 구워져 있다.

가중치가 로컬에 없으면 파일 전체를 건너뛴다 — `pytest` 한 줄이 수백 MB 다운로드에 묶이면
안 된다. 컨테이너 이미지에는 가중치가 구워져 있어 그 안에서는 이 층까지 돈다.

### `llm_judge.py` · `test_golden_judge.py` · `test_llm_judge.py`

골든셋 생성 채점 — 두 구성이 각자의 상위 K 로 만든 답변을 세 층으로 채점한다.

**채점 순서가 이 층의 전부다.** ① 검색 게이트(`evidence.quote` 가 상위 K 에 왔는가) →
② 결정적 문자열 검사(`expected_spans`·`must_not_contain`) → ③ LLM-Judge. 게이트를 통과하지
못한 문항은 생성 채점에서 빠지고 검색 실패로만 센다. 근거가 오지 않은 상태의 답변이 틀린
것은 생성기의 잘못이 아니고, 그걸 생성 실패로 세면 두 지표가 함께 움직여 진단이 불가능해진다
(골든셋 README 의 규칙이다).

**②와 ③을 합산하지 않는다.** ②는 결정적이지만 예상 밖 표현으로 틀린 답을 놓치고, ③은 그
구멍을 메우지만 흔들린다. 하나의 점수로 합치면 흔들리는 쪽이 결정적인 쪽을 오염시킨다.
둘이 엇갈린 문항은 `disagreements` 로 따로 찍는다 — 판정자를 의심할 자리가 거기다.

**게이트는 구성마다 따로 선다.** 교집합으로 좁히면 비교는 깨끗해지지만, 리랭킹이 근거를 상위
K 안으로 끌어올린 문항이 통째로 빠져 **이 단계가 값을 만든 자리를 측정이 지운다.** 실제로 첫
회차가 그랬다(design Open Questions). 분모가 갈리는 대신 그 차이 자체를 결과로 읽는다.

**종료 사유는 게이트와 무관하게 전부 기록한다.** 근거가 오지 않은 문항에서 시스템이 답을
만들었는지(`stop`) 거절했는지(`insufficient_evidence`)가 그 값에서만 보이고, 판정표에는
나타나지 않는다 — 그 문항들은 채점 대상이 아니기 때문이다.

**근거 0건은 실패가 아니라 정상 종료다.** 그때 `QaService` 는 생성기를 부르지 않고
`FinishReason.NO_EVIDENCE` 로 끝내며 본문을 만들지 않는다(`Answer.no_evidence`). 조각
이벤트만 읽으면 그 종료가 생성 실패와 같은 모양이라, 종료 이벤트를 읽고 `NO_EVIDENCE_TEXT`
한 문장으로 옮겨 판정자에게 넘긴다. `answerable: false` 6문항이 정확히 이 경로를 탄다.

**품질을 단언하지 않는다.** 단언하는 것은 하네스 건전성뿐이다 — 게이트가 비지 않았는가,
판정불가가 과반이 아닌가, 두 구성이 같은 문항을 풀었는가. 재지 않은 임계값을 회귀 게이트로
쓰지 않는다는 규칙을 여기에도 적용했다.

순수한 층(프롬프트 조립·판정 파싱·문자열 검사·로더)은 `test_llm_judge.py` 가 **기본 실행**
에서 검증한다. 구독 없이 도는 층이라 판정불가를 오답으로 세는 종류의 사고가 여기서 걸린다.
...
1. **키의 정체성** — 무엇에 반응하고 무엇에 반응하지 않는가. 표기만 다른 같은 질문이 같은
   키를 받아야 L1 이 성립하고, 여섯 재료 중 하나만 달라도 키가 갈려야 프롬프트·모델·색인을
   바꾼 뒤 옛 답변이 새 구성의 답인 척 남지 않는다. 그리고 키가 질의를 복원 가능한 형태로
   담지 않아야 한다 — 캐시 키는 `redis-cli KEYS` 와 로그에 그대로 노출되는 문자열이다.

   **리랭커 서명만 파라미터가 두 줄이다.** 이 재료에는 축이 둘 있어서다 — 끈 구성(빈
   문자열)과 켠 구성이 갈리는 것, 그리고 켠 구성끼리 모델이 다르면 갈리는 것. 앞은
   `APP_RERANKER_ENABLED` 토글이 실제로 일어나는 자리이고, 뒤는 모델 교체가 캐시에
   가려 관측되지 않는 자리라, 한 줄로 합치면 둘 중 하나만 확인된다.
...
**순위 신호는 세 필드가 함께 왕복해야 한다** — 청크의 `rerank_score` 와 항목의
`ordered_by`·`reranker`. `rerank_score` 만 실으면 히트가 리랭킹 점수를 든 채 융합
순서라고 말하고, 그 조합은 `/search` 가 낼 수 없는 응답이라 소비자의 분기가 하나 는다.
리랭킹 이전에 쓰인 항목은 이 셋이 없는데, 세대를 올리지 않고 없는 값을 "리랭킹이 돌지
않았다"로 읽는다 — 올리면 되살릴 수 있는 항목까지 미스가 된다.
