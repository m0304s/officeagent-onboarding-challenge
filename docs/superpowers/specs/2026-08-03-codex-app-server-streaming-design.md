...
전부 구현하고 실물 CLI로 돌려 본 뒤 이 문서를 다시 읽었다. **결정 1~12는 그대로 섰다** —
델타를 그대로 흘리는 것, 세션 풀, 반납 불변식, `core/`의 판정 줄 분리, 실패 정규화 셋,
테스트 다섯 겹까지 설계한 모양으로 구현됐고 실물에서 26개 델타·인용 검증·`INSUFFICIENT`
판정이 모두 관측됐다.

바로잡을 것 하나와 덧붙일 것 둘이 나왔다.

- **[정정] "첫 `/qa`는 여전히 12초대다"** (위 "위험과 대가") — 실측은 **7.4~9.8초**다.
  `thread/start` 7.87초는 그 회차가 세션을 처음 띄운 경우이고, 풀이 따뜻해진 뒤의 턴은
  4.5~8.0초다. 방향(첫 요청이 느리다)은 맞지만 숫자를 그대로 두면 상한 조정의 근거가 된다.
- **[덧붙임] 전송 계층이 서비스의 실행 문맥을 쪼개면 안 된다.** 이 문서가 다루지 않은
  자리에서 났다 — SSE 하트비트를 끼우려고 `__anext__`마다 태스크를 만들었더니 서비스가 든
  `anyio.CapacityLimiter`가 터졌다(빌린 태스크와 반납하는 태스크가 다르다). 결정 2의 풀
  상한과 서비스 세마포어가 같은 수를 두 번 세지 않는다는 성질은 **한 태스크 안에서만**
  성립한다. 지금은 서비스 스트림 전체를 한 태스크에서 돌려 큐로 옮긴다.
- **[덧붙임] 실물 층이 실제로 생겼다.** `tests/test_llm_live.py`(`-m llm`, 기본 제외)가
  델타가 여럿인지·판정 줄을 지키는지·근거를 인용하는지, 그리고 두 번째 턴이 세션을
  재사용하는지를 본다. 자격증명이 없으면 사유와 함께 건너뛴다.
